...
def test_update_command_uses_default_prompt_when_missing(monkeypatch: pytest.MonkeyPatch) -> None:
    config = Config(
        deck="Test",
        note_model="English",
        field_map=DEFAULT_FIELD_MAP,
    ...
    assert captured["sentence"] == "I run."
    assert captured["word"] == "run"
    assert captured["prompt"] == update_module.DEFAULT_UPDATE_PROMPT
    assert captured["update_note_fields"]["note_id"] == 123
