...
def test_update_command_overrides_context_source_from_prompt(monkeypatch: pytest.MonkeyPatch) -> None:
    config = Config(
        deck="Test",
        note_model="English",
        field_map=DEFAULT_FIELD_MAP,
        ...
        return [
            {
                ...
                "fields": {
                    "Word": {"value": "run"},
                    "Context Sentence": {"value": "Derived sentence."},
                    "Context Sentence Source": {"value": "Original source."},
                },
            }
        ]

    def fake_generate_card(_sentence: str, _word: str, **_kwargs: Any) -> Card:
        card = _sample_card()
        return Card(
            **{**card.as_dict(), "context_source": "Model source."}
        )
    ...
    assert captured["update_note_fields"]["note_id"] == 123
    assert captured["update_note_fields"]["fields"]["Context Sentence Source"] == "Original source."
